...
// Blacksmith GY to Lumber Mill
BattleBotPath vPath_AB_BlacksmithGY_to_LumberMill =
{
...
// Blacksmith GY to Gold Mine
BattleBotPath vPath_AB_BlacksmithGY_to_GoldMine =
{
...
bool BGTactics::selectObjective(bool reset)
{
    ...
    ai::PositionEntry pos = context->GetValue<ai::PositionMap&>("position")->Get()["bg objective"];
    if ((pos.isSet() && !reset) && !(bot->HasAura(BG_WS_SPELL_WARSONG_FLAG) || bot->HasAura(BG_WS_SPELL_SILVERWING_FLAG))) // flag carrier needs to be able to update objective or will get stuck
        return false;
